...
uint256 hashGenesisBlock("0x0000095fb77ee6ff3e640dcb1759d42a05e831c0893b8cf38581071ec39c4d5d");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // AUDW: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "AUDW Signed Message:\n";
...
{
    // AUDW: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 3 * 60; // AUDW: each block
static const int64 nTargetSpacing = 1 * 60; // AUDW: 300 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // AUDW: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, AUDW - DarkGravity, written by Evan Duffield - evan@AUDW.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // AUDW: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // AUDW: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        assert(block.hashMerkleRoot == uint256("0xc47c6b4486225f14337990dc7b8d41e4c49d96ea1c2e9223ce3f0fc47014ef49"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // AUDW: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AUDWMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AUDWMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AUDWMiner(CWallet *pwallet)
{
    printf("AUDWMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("AUDW-miner");
    ...
    try { while (true) {
        ...
        printf("Running AUDWMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AUDWMiner, pwallet));
}
